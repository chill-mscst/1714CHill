...
SELECT        Building.BuildingId, Building.BuildingName, Building.City, Apartment.ApartmentNum, Person.LastName, Person.FirstName
...
SELECT        Apartment.BuildingId, Apartment.ApartmentId, Person.LastName, Person.FirstName, Invoice.InvoiceId, Invoice.InvoiceDate, Receipt.Amount
FROM            Apartment INNER JOIN
                         Invoice ON Apartment.ApartmentId = Invoice.ApartmentId INNER JOIN
...
SELECT        Invoice.InvoiceId, Invoice.InvoiceDate, Person.LastName AS Tenant, Apartment.BuildingId, Apartment.ApartmentNum, Receipt.Amount AS Billed, Receipt.Amount AS Received
FROM            Invoice INNER JOIN
                         Apartment ON Invoice.ApartmentId = Apartment.ApartmentId INNER JOIN
                         Receipt ON Invoice.InvoiceId = Receipt.InvoiceId INNER JOIN
                         Person ON Apartment.TenantId = Person.PersonId
